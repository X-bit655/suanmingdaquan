function buildZiWeiPrompt(input, data) {
  ...
  const mgStars = mg ? [].concat(
    mg.majorStars.map(function (s) { return s.name }),
    mg.minorStars.map(function (s) { return s.name })
  ).join('、') : ''

  const palaceList = data.palaces.map(function (p) {
    const stars = [].concat(
      p.majorStars.map(function (s) { return s.name + '(' + s.brightness + ')' }),
      p.minorStars.filter(function (s) { return s.name }).map(function (s) { return s.name + '(' + s.brightness + ')' })
    )
