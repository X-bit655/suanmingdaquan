const FBazi = require("./prompts/bazi"); const FZhouYi = require("./prompts/zhouyi"); const FZiWei = require("./prompts/ziwei");
const FAstrology = require("./prompts/astrology"); const FTarot = require("./prompts/tarot"); const FSystem = require("./prompts/system");
const { chatCompletion } = require("./utils/deepseek-client"); const { parseResponse } = require("./utils/response-parser");

exports.main = async (event, context) => {
  const { method, input, calculatedData } = event;
  if (!method || !calculatedData) return { code: 400, message: "Missing params", data: null };
  ...
  switch (method) {
    ...
    default: return { code: 400, message: "Unknown method", data: null };
  }
  try {
    ...
    console.error("Fortune error:", e.message);
    return { code: 500, message: e.message || "AI service error", data: null };
  }
}
